...
function SortableAppItem({ app, onRemove, isCollapsed }) {
    const {
        ...
        id: typeof app.appId === "object" && app.appId ? app.appId._id : app.appId || `temp-${Math.random()}`,
    });
        ...
        return userApps.some(
            (userApp) =>
                ...
                (typeof userApp.appId === "object" &&
                    userApp.appId && userApp.appId._id === appId),
        );
    };
                    ...
                    <div className="p-6">
                        <Tabs defaultValue="available" className="w-full">
                            <TabsList className="grid w-full grid-cols-2">
                                <TabsTrigger value="available">{t("Available Apps")}</TabsTrigger>
                                <TabsTrigger value="your-apps">{t("Your Apps")}</TabsTrigger>
                            </TabsList>
                            ...
                            <TabsContent value="available" className="mt-6">
                                ...
                                <div className="mb-6">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            placeholder={t("Search apps by name or author...")}
                                            value={searchQuery}
                                                ...
                                                <div
                                                    ...
                                                    onClick={() => {
                                                        if (app.type === "applet" && app.workspaceId) {
                                                            router.push(`/published/workspaces/${app.workspaceId}/applet`);
                                                        } else if (app.slug) {
                                            ...
                                            {appletApps.map((app) => {
                                                ...
                                                {app.author && (
                                                    <div className="text-xs text-gray-500 rtl:text-right">
                                                        {t("by {{author}}", { author: app.author.username })}
                                                    </div>
                                                )}
                                                {app.updatedAt && (
                                                    <div className="text-xs text-gray-400 rtl:text-right mt-1 truncate">
                                                        {t("Updated")} {new Date(app.updatedAt).toLocaleString()}
                                                    </div>
                                                )}
                                                    </div>
                                                );
