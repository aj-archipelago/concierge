...
const Change = ({ token, onTokenChange }) => {
    ...
    if (addedValue && removedValue) {
        ...
        if (accepted) {
            ...
            changeMarkup = (
                <>
                    Add{" "}
                    <span style={{ ...green }}>&quot;{addedValue}&quot;</span>
                </>
            );
        }
    ...
    return (
        <ClickableToken className="change-button w-full mb-3 flex gap-2">
            <Checkbox defaultChecked={accepted} tabIndex="-1" />
...
const Diff = ({ string1 = "", string2 = "", setSelectedText }) => {
    // Initialize window.diff on component mount
    useEffect(() => {
    ...
    const tokens = groupedTokens.map((group, i) => {
        if (group.type === "change") {
            ...
            return (
                <span key={`change-group-${i}`} className="change-group">
                    <ChangeToken
