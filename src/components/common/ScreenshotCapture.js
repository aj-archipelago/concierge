    ...
    (
        ...
        const startScreenCapture = useCallback(async () => {
            ...
            try {
                ...
                if (error.name === "NotAllowedError") {
                    throw new Error(
                        "Screen capture permission was denied. You can try again or upload an image instead.",
                    );
                    ...
                    throw new Error(
                        "Unable to read from screen capture device. Please try again.",
                    );
                } else {
                    throw new Error(
                        "Failed to start screen capture. Please try again or upload an image instead.",
                    );
                }
            }
        }, [displaySurface]);
        ...
        const captureFrame = useCallback(async (stream) => {
            ...
            while (imageData.length > MAX_IMAGE_SIZE && attempts > 0) {
                ...
                attempts--;
                console.log(
                    `Compressing image, attempt ${3 - attempts}, size: ${Math.round(imageData.length / 1024)}KB`,
                );
            }
        ...
        const captureScreenshot = useCallback(async () => {
            try {
                ...
                const stream =
                    activeStreamRef.current || (await startScreenCapture());

                // Capture frame from stream
                const imageData = await captureFrame(stream);

                console.log(
                    `Screenshot captured (size: ${Math.round(imageData.length / 1024)}KB)...`,
                );
