...
describe('URL Validation Functions', () => {
    ...
    describe('isValidUrl', () => {
        it('should return true for valid URLs', () => {
            ...
            expect(isValidUrl('https://storage.googleapis.com/bucket/video.mp4')).toBe(true);
        });
    ...
    describe('isCloudStorageUrl', () => {
        it('should identify Azure Blob Storage URLs', () => {
            expect(isCloudStorageUrl('https://myaccount.blob.core.windows.net/container/video.mp4')).toBe(true);
        });

        it('should identify Google Cloud Storage URLs', () => {
            expect(isCloudStorageUrl('https://storage.googleapis.com/bucket/video.mp4')).toBe(true);
            expect(isCloudStorageUrl('https://storage.cloud.google.com/bucket/video.mp4')).toBe(true);
        });

        it('should identify AWS S3 URLs', () => {
            expect(isCloudStorageUrl('https://bucket.s3.amazonaws.com/video.mp4')).toBe(true);
            expect(isCloudStorageUrl('https://bucket.s3-us-west-2.amazonaws.com/video.mp4')).toBe(true);
        });

        it('should identify AWS CloudFront URLs', () => {
            expect(isCloudStorageUrl('https://d1234.cloudfront.net/video.mp4')).toBe(true);
        });

        it('should identify DigitalOcean Spaces URLs', () => {
            expect(isCloudStorageUrl('https://bucket.digitaloceanspaces.com/video.mp4')).toBe(true);
        });

        it('should return false for non-cloud storage URLs', () => {
            expect(isCloudStorageUrl('https://example.com/video.mp4')).toBe(false);
            expect(isCloudStorageUrl('https://youtube.com/watch?v=123')).toBe(false);
        });
    });
});
...
describe('checkVideoUrl', () => {
    ...
    it('should validate a regular video URL successfully', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://example.com/video.mp4', { method: 'HEAD' });
    });

    it('should skip HEAD request for cloud storage URLs', async () => {
        const result = await checkVideoUrl('https://storage.googleapis.com/bucket/video.mp4');
        expect(result).toBe(true);
    ...
    it('should reject non-video content types', async () => {
        ...
        const result = await checkVideoUrl('https://example.com/not-a-video.html');
    ...
    it('should handle videos longer than 60 minutes', async () => {
        ...
        const result = await checkVideoUrl('https://example.com/long-video.mp4');
