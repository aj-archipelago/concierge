...
const isMediaHelperConfigured = () => {
    try {
        return process.env.CORTEX_MEDIA_API_URL || (getConfig()?.endpoints?.mediaHelper && typeof getConfig().endpoints.mediaHelper === 'function');
    } catch (error) {
        return false;
    }
};
...
async function processImageUrls(message, serverUrl) {
    ...
    while ((match = urlRegex.exec(message)) !== null) {
        if (match[2]) {
            ...
            matches.push({ url: match[2], description: match[1], fullMatch: match[0] });
        } else {
            // It's a regular URL
            matches.push({ url: match[0], description: null, fullMatch: match[0] });
        }
    }
    ...
    for (const { url, description, fullMatch } of matches) {
        if (isImageUrl(url)) {
            try {
                ...
                const timeoutId = setTimeout(() => controller.abort(), MEDIA_HELPER_TIMEOUT_MS);
                ...
                try {
                    ...
                    if (uploadResponse.ok) {
                        ...
                        if (description !== null) {
                            ...
                            message = message.replace(fullMatch, `![${description}](${data.url})`);
                        } else {
                            // Replace regular URL
                            message = message.replace(url, data.url);
                        }
                        console.log('Replaced temporary image URL with permanent URL:', data.url);
                    }
                } catch (fetchError) {
                    clearTimeout(timeoutId);
                    if (fetchError.name !== 'AbortError') {
                        console.error("Error fetching from media helper:", fetchError);
                    }
                }
            } catch (error) {
