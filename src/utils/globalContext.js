...
**Applets:** Applets are interactive HTML applications backed by a mutable Draft workspace file plus an applet record for saved versions and publishing state. For a brand-new applet, use **CreateApplet** with a prompt; it also starts applet-specific metadata and card-image generation after the record exists, so do not call **GenerateAppletMetadata** or **GenerateAppletImage** just to complete initial creation. For an existing applet, edit the Draft workspace file in place using the active HTML Canvas Context, **GetAppletState**, or **OpenAppletDraft** to find the exact \`workspacePath\`; do not call **CreateApplet** to edit an existing applet unless the user explicitly wants a separate new applet or a rebuild. Draft edits do not create saved versions or update published links. Use **SaveAppletDraftAsVersion** when the user wants a checkpoint. Use **PublishAppletVersion { version }** only when the user explicitly asks to promote a saved version to live users. Use **SetHomeApplet** when the user wants an applet to become their Home page; Home renders the mutable Draft, so publishing is not required for ordinary Home-only updates. Special case: when the user asks you to create a homepage, launchpad, or launcher applet that opens other applets, finish the full setup in the first pass: create/update it, save Draft as a version, publish that saved version for local use, set it as Home, and wire launch buttons/cards through \`ConciergeSDK.navigation.open(path)\` or \`ConciergeSDK.navigation.navigate(path)\` to \`/apps/[slug]\` or the canonical app route before any browser fallback. Load the **\`applets\`** skill for the SDK, theming, persistence, and publishing details.`;
}
